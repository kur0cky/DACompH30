...
NMFは得られた非負行列を以下のように二つの非負値行列に分解することを前提としている．
...
watch_rate %>% 
  ...
  left_join(pro, by = c("station_code", "program_start_time")) %>% 
  select(program_code, all_watch_rate) %>% 
  group_by(program_code) %>% 
  summarise(all_watch_rate = mean(all_watch_rate)) %>%
  right_join(basis9, by = "program_code") %>% 
  group_by(topic) %>% 
  filter(value > mean(value)) %>% 
  ggplot(aes(all_watch_rate))+
  geom_histogram()+
  facet_wrap(~topic)
