...
タイムシフト視聴の，もとの放送timestamp

- min : 2017-03-27 05:30:00
- max : 2018-04-02 02:37:00
- minを見れば分かるが, 計測期間以前の放送が当然ある

```{r}
play %>% 
  rename(timeshift_datetime = timesfhit_datetime) %>% 
  summarise(max_timeshift_datetime = max(timeshift_datetime),
            min_timeshift_datetime = min(timeshift_datetime))
```
